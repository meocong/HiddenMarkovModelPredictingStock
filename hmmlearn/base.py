...
class _BaseHMM(BaseEstimator):
    ...
    def _init(self, X, lengths):
        ...
        if 's' in self.init_params or not hasattr(self, "startprob_"):
            ...
            # Changed by Viet
            self.startprob_ = np.array(np.random.dirichlet(np.ones(self.n_components),size=1)[0])
        ...
        if 't' in self.init_params or not hasattr(self, "transmat_"):
            ...
            # Changed by Viet
            self.transmat_ = np.array([np.random.dirichlet(np.ones(self.n_components),size=1)[0] for x in range(self.n_components)])
