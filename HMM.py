...
def get_value_by_dates(df, datestart, dateend):
    ...
    return np.column_stack([(close_v - open_v)/open_v, volume_v, (high_v - open_v)/open_v, (low_v - open_v)/open_v]), dates, close_v, volume_v, high_v, open_v, low_v

def show_plot(model, dates, close_v, hidden_states):
    ###############################################################################
...
model = GaussianHMM(algorithm='viterbi', n_components=3, covariance_type="full", n_iter=10000).fit(X)
...
show_plot(model, dates, close_v, hidden_states)
...
show_plot(model, dates, close_v, predicted)
print(predicted)
